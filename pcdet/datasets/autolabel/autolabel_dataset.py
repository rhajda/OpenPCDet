...
class AutolabelDataset(DatasetTemplate):
    ...
    def include_kitti_data(self, mode):
        if self.logger is not None:
            self.logger.info(f"Loading autolabel dataset using samples defined in {self.split}.txt")
        else:
            print(f"Loading autolabel dataset using samples defined in {self.split}.txt")
        kitti_infos = []
        ...
        if self.logger is not None:
            self.logger.info('Total samples for autolabel dataset: %d' % (len(kitti_infos)))
        else:
            print('Total samples for autolabel dataset: %d' % (len(kitti_infos)))
    ...
    def remove_missing_gt_samples(self):
        ...
        if self.logger is not None:
            self.logger.info('Total samples for autolabel dataset after optimizing: %d' % (len(self.kitti_infos)))
...
if __name__ == '__main__':
    import sys
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_autolabel_infos':
        import yaml
        from pathlib import Path
        from easydict import EasyDict
        dataset_cfg = EasyDict(yaml.safe_load(open(sys.argv[2])))
        ROOT_DIR = (Path(__file__).resolve().parent / '../../../').resolve()
        create_autolabel_infos(
            dataset_cfg=dataset_cfg,
            class_names=['Car'],
            data_path=ROOT_DIR / 'data' / 'autolabel',
            save_path=ROOT_DIR / 'data' / 'autolabel'
        )
