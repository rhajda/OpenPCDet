...
class S2rDataset(DatasetTemplate):
    ...
    def include_kitti_data(self, mode):
        if self.logger is not None:
            self.logger.info(f"Loading s2r dataset using samples defined in {self.split}.txt")
        else:
            print(f"Loading s2r dataset using samples defined in {self.split}.txt")
        kitti_infos = []
        ...
        if self.logger is not None:
            self.logger.info('Total samples for s2r dataset: %d' % (len(kitti_infos)))
        else:
            print('Total samples for s2r dataset: %d' % (len(kitti_infos)))
    ...
    def remove_missing_gt_samples(self):
        ...
        if self.logger is not None:
            self.logger.info('Total samples for s2r dataset after optimizing: %d' % (len(self.kitti_infos)))
...
def create_s2r_infos(dataset_cfg, class_names, data_path, save_path, workers=16):
    ...
    train_means = save_path / ('train_means.txt')
    val_means = save_path / ('val_means.txt')
...
if __name__ == '__main__':
    import sys
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_s2r_infos':
        import yaml
