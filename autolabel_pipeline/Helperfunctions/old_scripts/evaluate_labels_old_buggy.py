...
from pcdet.datasets.autolabel.kitti_object_eval_python.eval import get_thresholds, clean_data, image_box_overlap, calculate_iou_partly, bev_box_overlap, d3_box_overlap_kernel, d3_box_overlap, get_split_parts, _prepare_data, get_mAP, get_mAP_R40, print_str
...
# Function from pcdet, adapted for autolabel
def eval_class(gt_annos, dt_annos, current_classes, difficultys, metric, min_overlaps, FN_empty_det_frames, compute_aos=False, num_parts=100):
        ...
        for l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                for i in range(len(thresholds)):
                    ...
                    # pr = [[TP, FP, FN, TN]]
                    ###############
                    recall[m, l, k, i] = pr[i, 0] / (pr[i, 0] + (pr[i, 2] + (list(FN_empty_det_frames.values())[m]) ))
...
# Function from pcdet, adapted for autolabel
def do_eval(gt_annos, dt_annos, current_classes, min_overlaps, FN_empty_det_frames, compute_aos=False, PR_detail_dict=None):
    ...
    '''
    ret, confusion_mat = eval_class(gt_annos, dt_annos, current_classes, difficultys, 2, min_overlaps, FN_empty_det_frames)
...
# Function from pcdet, adapted for autolabel
def get_official_eval_result(gt_annos, dt_annos, current_classes, FN_empty_det_frames, PR_detail_dict=None):
    ...
    mAPbbox, mAPbev, mAP3d, mAPaos, mAPbbox_R40, mAPbev_R40, mAP3d_R40, mAPaos_R40, confusion_mat = do_eval(
        gt_annos, dt_annos, current_classes, min_overlaps, FN_empty_det_frames, compute_aos, PR_detail_dict=PR_detail_dict)
...
def eval_with_empty_detection_frames(gt_annos, dt_annos):
    ...
    if len(empty_elements) == 0:
        result, ret_dict, my_eval_dict = get_official_eval_result(gt_annos, dt_annos, cfg.PIPELINE.CLASSES, FN_counts)
        return result, ret_dict, my_eval_dict, FN_counts, empty_frame_list
...
def compute_performance_metrics(frame_IDs):
    ...
    result, ret_dict, my_eval_dict, FN_counts, empty_frame_list = eval_with_empty_detection_frames(gt_annos, dt_annos)
...
if __name__ == "__main__":
    ...
    folder_2 = cfg.DATA.PATH_PTRCNN_PREDICTIONS

    #frame_IDs = [62, 314, 394, 499, 878, 932, 1019, 1344, 1650, 1752, 2075, 2136, 2220, 2325, 2628, 2764, 2863]
    frame_IDs = common_set_between_datasets(cfg, [folder_1, folder_2], False) #important: False when handling ground truths

    frame_IDs = [num for num in frame_IDs if num >= 7000]
